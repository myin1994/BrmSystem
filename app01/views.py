...
class Login(View):
    ...
    def post(self, request):
        ...
        if obj.is_valid():
            ...
            print(models.UserInfo.objects.get(username=request.POST.get("username")).id)
...
class Customers(View):
    def get(self, request):
        ...
        search_uesr = request.GET.get('uid')#查询者id
        #获取查询总对象
        if search_model and search_value:
            all_customers = models.Customer.objects.filter(is_delete=False,**{search_model: search_value})
        else:
            all_customers = models.Customer.objects.filter(is_delete=False)
        #判断是否查询用户个人数据
        if search_uesr:
            all_customers = all_customers.filter(consultant__id=search_uesr)
        #获取页数
...
class CustomerEditOrAdd(View):
    def get(self, request, cid=None,page=None):
    ...
    def post(self, request, cid=None,page=None):
        obj_list = models.Customer.objects.filter(id=cid)
        ...
        if customer_modelform_obj.is_valid():
            ...
            return redirect(reverse('app01:customers')+f'?page={page}')
        else:
            return render(request, 'sales/customer_add.html', locals())


def CustomerDel(request, cid,page=None):
